...
Gitlab CI has very smart system of inheritance, if you're not happy with some part of process - you can just 
jump in and override that part with your specific declareation.

In following example I'm gonna show you how to add [MariaDB to the test step](https://dev.to/mstrsobserver/ci-testing-nodejs-application-with-mysql-database-3l7c).
...
variables:
  ...
  DOCKERIZE_RELEASE_URL: https://github.com/jwilder/dockerize/releases/download/v0.6.1/dockerize-alpine-linux-amd64-v0.6.1.tar.gz
...
test:delivery: # stage: test
  ...
  services:
    - name: mariadb:latest
      alias: mysql
  script:
    - wget -q "${DOCKERIZE_RELEASE_URL}" -O dockerize.tar.gz
    - tar -xvf dockerize.tar.gz && rm -rf dockerize.tar.gz
